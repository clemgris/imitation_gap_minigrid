...
class MetaMaze(environment.Environment):
    ...
    def get_obs(self, state: EnvState, params: EnvParams, is_expert: bool = False) -> chex.Array:
        ...
        agent_pos = state.pos.reshape(-1)
        condition_x = jnp.logical_and(
            state.pos[0] - self.rf_off <= state.goal[0],
            state.goal[0] <= state.pos[0] + self.rf_off
        )

        condition_y = jnp.logical_and(
            state.pos[1] - self.rf_off <= state.goal[1],
            state.goal[1] <= state.pos[1] + self.rf_off
        )

        is_goal_in_rf = jnp.logical_and(condition_x, condition_y)
        show_goal = jnp.logical_or(is_expert, is_goal_in_rf)
        goal_pos = jnp.where(show_goal, state.goal, -1 * jnp.ones_like(state.goal)).reshape(-1)

        return jnp.hstack([rf_obs, agent_pos, goal_pos])
    ...
    def observation_space(self, params: EnvParams) -> spaces.Box:
        ...
        return spaces.Box(
            low, high, (self.rf_size ** 2 + 2 + 2,), jnp.float32
        )
